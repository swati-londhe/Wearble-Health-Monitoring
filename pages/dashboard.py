...
def fetch_google_fit_data(creds):
    ...
    # Fetch data for the last 2 hours to cover recent updates
    ...
    heart_rate_available = any("heart_rate" in ds.get("dataType", {}).get("name", "").lower() for ds in sources)
    ...
    body = {
        ...
        "bucketByTime": {"durationMillis": 60000},  # 5-minute buckets
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if attempt < max_retries - 1:
                    ...
                    st.error(f"Error fetching data after {max_retries} attempts: {response.text}")
                    break
        except requests.exceptions.RequestException as e:
    ...
    for bucket in data.get("bucket", []):
        ...
        dt = datetime.fromtimestamp(start_time_millis / 1000)
        rounded_dt = round_to_nearest_5min(dt)
        timestamp = rounded_dt.strftime('%Y-%m-%d %H:%M:%S')
    ...
    # Ensure the latest 5-minute bucket is present
    current_time = now.replace(second=0, microsecond=0)
    current_5min = round_to_nearest_5min(current_time)
    latest_timestamp = rows[-1]["Timestamp"] if rows else None

    if not latest_timestamp or datetime.strptime(latest_timestamp, '%Y-%m-%d %H:%M:%S') < current_5min:
        rows.append({
    ...
    if any(row["Steps"] is not None or row["Heart Rate (BPM)"] is not None or row["SpO2 (%)"] is not None for row in rows):
        st.info(f"Fetched {len(rows)} data points. Latest timestamp: {rows[-1]['Timestamp']}")
    else:
        st.warning("No valid data values (Steps, Heart Rate, SpO2) found in the response.")
    ...
    if rows:
        ...
        if os.path.exists(CSV_FILE):
            try:
                ...
                old_df['Timestamp'] = pd.to_datetime(old_df['Timestamp']).apply(round_to_nearest_5min).dt.strftime('%Y-%m-%d %H:%M:%S')
                combined = pd.concat([old_df, new_df], ignore_index=True)
                combined = combined.sort_values(by="Timestamp").drop_duplicates(subset=["Timestamp"], keep="last")
                combined.to_csv(CSV_FILE, index=False)
            except pd.errors.ParserError as e:
                st.error(f"Error parsing CSV file: {e}. Using new data only.")
